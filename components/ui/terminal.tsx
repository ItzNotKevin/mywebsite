  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        const res = await fetch("/sounds/Sound from Aceternity.ogg");
        if (!res.ok) return;
...
export function Terminal({
  ...
  useEffect(() => {
    ...
    else if (isLastCommand) { setPhase("done"); onComplete?.(); }
  ...
  useEffect(() => {
    ...
    if (outputIdx >= 0 && outputIdx < currentOutputs.length) {
      ...
      const t = setTimeout(() => {
        if (isLastCommand) { setPhase("done"); onComplete?.(); } else { setPhase("pausing"); }
      }, 300);
      return () => clearTimeout(t);
    }
  ...
  return (
    <div ref={containerRef} className={cn("mx-auto w-full max-w-xl px-4 font-mono text-xs", className)}>
      <div className="overflow-hidden rounded-lg border border-neutral-800 bg-neutral-900 shadow-2xl">
        ...
        {/* Terminal Content */}
        <div ref={contentRef} className="no-visible-scrollbar h-[28rem] overflow-y-auto p-4 font-mono">
          {lines.map((line, i) => (
